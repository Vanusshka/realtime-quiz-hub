...
void main() {
  ...
  if (uColorCount > 0) {
    ...
    // Create flowing, animated rings
    float ringPattern = dist * uFrequency;
    ringPattern += sin(angle * 4.0 + t * 2.0) * 0.2; // Wavy rings
    ringPattern -= t * 1.5; // Animate outward
    ...
    for (int i = 0; i < MAX_COLORS; ++i) {
      ...
      // Create distinct rings for each color
      float ringOffset = float(i) * 1.5;
      float ringDist = mod(ringPattern + ringOffset, float(uColorCount) * 1.5);
      
      // Create ring with soft edges
      float w = smoothstep(1.5, 0.5, ringDist) * smoothstep(0.0, 0.5, ringDist);
      w = pow(w, 1.5); // Sharpen the rings
...
export default function ColorBends({
  ...
  useEffect(() => {
    ...
    // Convert colors immediately on mount
    ...
    console.log('ColorBends: Initializing with', colorArr.length, 'colors');
    for (let i = 0; i < colorArr.length; i++) {
      uColorsArray[i].copy(colorArr[i]);
