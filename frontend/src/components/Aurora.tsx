...
void main() {
  ...
  vec3 rampColor;
  COLOR_RAMP(colors, uv.x + uTime * 0.02, rampColor);
  
  float height = snoise(vec2(uv.x * 2.5 + uTime * 0.15, uTime * 0.2)) * 0.5 * uAmplitude;
  height += snoise(vec2(uv.x * 1.2 - uTime * 0.1, uTime * 0.15)) * 0.25 * uAmplitude;
  height = exp(height);
